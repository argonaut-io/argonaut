...
object PrettyParamsSpecification extends ArgonautSpec {
  ...
  def objectCommaLeftIndent = prop { (indent: String) =>
    ...
    prettyParams.pretty(
      ("test" := "value") ->: ("test2" := "value2") ->: jEmptyObject
    ) === """{"test":"value"%s,"test2":"value2"}""".format(indent)
  }
  ...
  def numberPrintingFractionalNumber = forAll(
    arbitrary[(Double, Double)].filter { case (first, second) => second != 0 }
  ...
  case class NullKey(
    ...
    onlyNullKeyExpected: String,
    firstKeyNullExpected: String,
    middleKeyNullExpected: String,
    lastKeyNullExpected: String
  ) {
    ...
    def firstKeyNull = prettyParams.pretty(
      ("testNullKey" := jNull) ->: ("test" := "value") ->: ("testTwo" := "valueTwo") ->: jEmptyObject
    ) === firstKeyNullExpected
    def middleKeyNull = prettyParams.pretty(
      ("test" := "value") ->: ("testNullKey" := jNull) ->: ("testTwo" := "valueTwo") ->: jEmptyObject
    ) === middleKeyNullExpected
    def lastKeyNull = prettyParams.pretty(
      ("test" := "value") ->: ("testTwo" := "valueTwo") ->: ("testNullKey" := jNull) ->: jEmptyObject
    ) === lastKeyNullExpected
  }
}
