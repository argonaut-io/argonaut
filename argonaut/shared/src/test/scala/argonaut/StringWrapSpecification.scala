...
object StringWrapSpecification extends ArgonautSpec {
  ...
  def is = s2"""
  parse
    Optional encode ${(("optional" :?= (None: Option[String])) ->?: jEmptyObject) must_== jEmptyObject}
    Optional encode alias ${prop { (o: Option[Int]) =>
      (("optional" :?= o) ->?: jEmptyObject) must_== (("optional" :?= o) ->?: jEmptyObject)
    }}
    Optional encode precedence ${("optional" :?= Some(1) filter (x => x > 0)) must_== Some("optional" := 1)}
    returns a success wrapped Json for valid JSON ${prop { (json: Json) =>
      json.nospaces.parse == Right(json)
    }}
    returns a failure for invalid JSON ${"{".parse == Left("JSON terminates unexpectedly.")}
    ...
    returns a failure for invalid JSON ${"{".parseWith(_ => "Oops", identity) == "JSON terminates unexpectedly."}
    ...
    decodeWith[A, X: DecodeJson](X => A, String => A, (String, CursorHistory) => A): A
      returns the decoded and transformed Json for valid JSON ${forAllNoShrink(alphaStr, arbitrary[Int]) {
      (name: String, age: Int) =>
        val json = validJSONTemplate.format(name, age)
      ...
      .decodeWith[Option[Person], Person](_ => None, _ => Some(Person("Test", 5)), (_, _) => None) == Some(
      Person("Test", 5)
    )}
      returns the result of the decodeFailure function for undecodable JSON ${forAllNoShrink(alphaStr, arbitrary[Int]) {
      (name: String, age: Int) =>
        val json = invalidJSONTemplate.format(name, age)
        json.decodeWith[Option[Person], Person](_ => None, _ => None, (_, _) => Some(Person("Test", 5))) == Some(
          Person("Test", 5)
        )
    }}
    ...
    decodeOr[A, X: DecodeJson](X => A, => A): A
      ...
      returns the result of the default function for undecodable JSON ${forAllNoShrink(alphaStr, arbitrary[Int]) {
      (name: String, age: Int) =>
        val json = invalidJSONTemplate.format(name, age)
    ...
    parseOr[X](Json => X, => X): X
      returns the transformed Json for valid JSON ${forAllNoShrink(alphaStr, arbitrary[Int]) {
      (name: String, age: Int) =>
        val json = validJSONTemplate.format(name, age)
        json.parseOr[Option[Json]](Some.apply, None) == Some(
          ("age", jString(age.toString)) ->: ("name", jString(name)) ->: jEmptyObject
        )
    }}
      ...
      returns a failure for invalid JSON ${"{".parseOption == None}
      decodeOption[X: DecodeJson]: Option[X]
      returns the decoded value wrapped in a Some for valid JSON ${forAllNoShrink(alphaStr, arbitrary[Int]) {
      (name: String, age: Int) =>
        val json = validJSONTemplate.format(name, age)
        json.decodeOption[Person] == Some(Person(name, age))
    }}
      returns a None for invalid JSON ${"{".decodeOption[Person] == None}
